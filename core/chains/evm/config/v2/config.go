...
func (c *Chain) ValidateConfig() (err error) {
	...
	if !*c.FinalityTag {
		if *c.HeadTracker.TotalUnfinalizedHeadsLimit < *c.FinalityDepth {
			err = multierr.Append(err, v2.ErrInvalid{Name: "HeadTracker.TotalUnfinalizedHeadsLimit", Value: *c.HeadTracker.TotalUnfinalizedHeadsLimit,
...
type HeadTracker struct {
	TotalUnfinalizedHeadsLimit *uint32
	MaxBufferSize              *uint32
	SamplingInterval           *models.Duration
}

func (t *HeadTracker) setFrom(f *HeadTracker) {
	if v := f.TotalUnfinalizedHeadsLimit; v != nil {
		t.TotalUnfinalizedHeadsLimit = v
	}
